    ...
    for i in range(num):
        for j in range(2):     # source : 0,  sink : 1
            ...
            while True:
                ...
                if not (temp_x_left_bot < 2 or temp_y_left_bot > 61 or \
                   temp_x_right_up < 2 or temp_y_right_up > 61 or \
                   temp_x_right_bot < 2 or temp_x_right_bot > 61) and \
                   temp_map[temp_y_left_up:temp_y_left_bot+1, temp_x_left_up:temp_x_right_bot+1].sum() == 0:
...
for i in range(32):
    for j in range (64):
        for k in range (8):      # 32 bits represent 8 elements  
            ...
            data = hexx(mapp[i][j][8*k+1]) + hexx(mapp[i][j][8*k+0]) + ' ' + \
                   hexx(mapp[i][j][8*k+3]) + hexx(mapp[i][j][8*k+2]) + ' ' + \
                   hexx(mapp[i][j][8*k+5]) + hexx(mapp[i][j][8*k+4]) + ' ' + \
                   hexx(mapp[i][j][8*k+7]) + hexx(mapp[i][j][8*k+6]) 
...
for i in range(32):
    for j in range (64):
        for k in range (8):      # 32 bits represent 8 elements  
            ...
            data = hexx(random.randint(1,15)) + hexx(random.randint(1,15)) + ' ' + \
                   hexx(random.randint(1,15)) + hexx(random.randint(1,15)) + ' ' + \
                   hexx(random.randint(1,15)) + hexx(random.randint(1,15)) + ' ' + \
                   hexx(random.randint(1,15)) + hexx(random.randint(1,15)) 
